...
-- 형 변환함수 TO_CHAR(문자열) TO_NUMBER(숫자) TO_DATE(날짜)
...
-- 주어진 자릿수에 숫자를 모두 표기 할 수 없어서 모두 #으로 표기됩니다.
...
SELECT sysdate - TO_DATE('2021-03-26') FROM dual; -- 날짜로 형 변환
SELECT TO_DATE('2020/12/25', 'YY-MM-DD') FROM dual;

-- 주어진 문자열을 모두 변환해야 합니다.
SELECT TO_DATE('2021-03-31 12:23:50', 'YY-MM-DD HH:MI:SS') FROM dual; -- 에러
...
SELECT
    first_name,
    NVL2(commission_pct, 'true', 'fales')
FROM employees;
...
SELECT
    ...
    hire_date,
    TO_CHAR(sysdate,'YY')-
    TO_CHAR(TO_DATE(hire_date,'YY-MM-DD'),'YY') AS 근속년수
--    TRUNC((sysdate - hire_date) / 365) AS 근속년수
FROM employees
WHERE  TO_CHAR(sysdate,'YY')-
    TO_CHAR(TO_DATE(hire_date,'YY-MM-DD'),'YY') BETWEEN 17 AND 999
ORDER BY TO_CHAR(sysdate,'YY')-
    TO_CHAR(TO_DATE(hire_date,'YY-MM-DD'),'YY') DESC;
