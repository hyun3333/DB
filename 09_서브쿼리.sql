...
SELECT salary FROM employees WHERE first_name = 'Daivd';
...
-- 스칼라 서브쿼리
-- SELECT 구문에 서브쿼리가 오는 것. LECT OUTER JOIN과 유사한 결과.
SELECT
...
-- ROWNUM을 붙이고 나서 범위를 지정해서 조히하려고 하는데,
...
/*
가장 안쪽 SELECT 절에서 필요한 테이블 형식(인라인 뷰)을 형성.
...
SELECT * FROM
    ...
    FROM
        ...
        SELECT '박뚜띠', '20230401' FROM dual UNION ALL
        SELECT '김성원', '18180118' FROM dual
        )
    )
WHERE mm = '0118';
